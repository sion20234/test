...
{
	/// <summary>4-5-23 ガイド番号設定コマンド</summary>
	public class McReqGuideNumber : McCommBasic, IEcnc3McCommand, IDisposable
    {
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;
		/// <summary>コンストラクタ</summary>
		public McReqGuideNumber()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_GUIDENOCHG;
			DataTypeName = "REQ_GUIDENOCHG";
		}
		/// <summary>ガイド番号</summary>
		...
		{
			AidLog logs = new AidLog( "McReqSettingGuideNumber.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					GUIDENO data = GUIDENO.Init();
					data.GuideNo = GuideNumber;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},GuideNo={data.GuideNo}" );
