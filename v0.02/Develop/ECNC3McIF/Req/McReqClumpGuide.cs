...
{
	/// <summary>4-5-27 ガイド クランプ／アンクランプ コマンド</summary>
	public class McReqClumpGuide : McCommBasic, IEcnc3McCommand, IDisposable
    {
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;
		/// <summary>コンストラクタ</summary>
		public McReqClumpGuide()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_GUIDECLUMP;
			DataTypeName = "REQ_GUIDECLUMP";
		}
		...
		{
			AidLog logs = new AidLog( "McReqClumpGuide.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					GUIDECLUMP data = GUIDECLUMP.Init();
					data.clump = ( true == Clamped ) ? 1 : 0;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},clump={data.clump})" );
