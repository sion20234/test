...
{
	/// <summary>4-5-26.GSFアーム移動コマンド</summary>
	public class McReqGsfMoveArm : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqGsfMoveArm()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_MOVEGSFARM;
			DataTypeName = "REQ_MOVEGSFARM";
		}
		/// <summary>コマンド</summary>
		public GsfArmPositions Position { get; set; }
		...
		{
			AidLog logs = new AidLog( "McReqGsfMoveArm.Execute" );
			if( GsfArmPositions.Unknown == Position ) {
				return ResultCodes.InvalidArgument;
			}
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					MOVEGSFARM data = MOVEGSFARM.Init();
