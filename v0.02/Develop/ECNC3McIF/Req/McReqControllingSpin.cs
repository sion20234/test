...
{
	/// <summary>4-5-47.主軸回転ON/OFFコマンド</summary>
	public class McReqControllingSpin : McCommBasic, IEcnc3McCommand, IDisposable
    {
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqControllingSpin()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_SPCMND;
			DataTypeName = "REQ_SPCMND";
		}
		/// <summary>コマンド</summary>
		public SpinStates SpinAction { get; set; }
		...
		{
			AidLog logs = new AidLog( "McReqControllingSpin.Execute" );
			if( SpinStates.Unknown == SpinAction ) {
				return ResultCodes.InvalidArgument;
			}
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					SPCMND data = SPCMND.Init();
					data.SpOut = (short)SpinAction;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},SpOut={data.SpOut})" );
