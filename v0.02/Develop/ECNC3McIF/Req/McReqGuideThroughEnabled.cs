...
{
	/// <summary>4-5-35.ガイド貫通動作許可コマンド</summary>
	public class McReqGuideThroughEnabled : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqGuideThroughEnabled()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_GUIDETHROUGH;
			DataTypeName = "REQ_GUIDETHROUGH";
		}
		...
		{
			AidLog logs = new AidLog( "McReqGuideThroughEnabled.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					GUIDETHROUGH data = GUIDETHROUGH.Init();
					data.move = (short)( ( true == Enabled ) ? 1 : 0 );
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},move={data.move}" );
