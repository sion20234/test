...
{
	/// <summary>4-2-20.運転プログラム選択コマンド</summary>
	public class McReqProgramSelect : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;
		/// <summary>コンストラクタ</summary>
		public McReqProgramSelect()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_PROGSLCT;
			DataTypeName = "REQ_PROGSLCT";
		}
		...
		{
			AidLog logs = new AidLog( "McReqProgramSelect.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					PROGSEL data = PROGSEL.Init();
					data.ProgSel = ProgramNumber;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},Prg#={data.ProgSel}" );
