...
{
	/// <summary>4-5-24.ガイド装着／脱着動作開始コマンド</summary>
	public class McReqGuideInsall : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqGuideInsall()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_GUIDEINSTALL;
			DataTypeName = "REQ_GUIDEINSTALL";
		}
		...
		public ResultCodes Execute()
		{
			...
			while( true == StandBy() ) {
				try {
					GUIDEINSTALL data = GUIDEINSTALL.Init();
					data.install = (short)( ( true == Install ) ? 1 : 0 );
					data.GuideNo = GuideNumber;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},install={data.install},GuideNo={data.GuideNo})" );
