...
{
	/// <summary>4-5-43 手動パルサー動作許可コマンド</summary>
	public class McReqHandPulserEnabled : McCommBasic, IEcnc3McCommand, IDisposable
    {
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;
		/// <summary>コンストラクタ</summary>
		public McReqHandPulserEnabled()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_HANDLEPERMIT;
			DataTypeName = "REQ_HANDLEPERMIT";
		}
		...
		{
			AidLog logs = new AidLog( "McReqSettingHandPulserEnabled.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					HANDLEPERMIT data = HANDLEPERMIT.Init();
					data.permit = (short)( ( true == Enabled ) ? 1 : 0 );
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},permit={data.permit}" );
