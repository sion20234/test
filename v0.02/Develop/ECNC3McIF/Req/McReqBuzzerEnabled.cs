...
{
	/// <summary>4-5-53.ブザーON／OFF信号出力コマンド</summary>
	public class McReqBuzzerEnabled : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqBuzzerEnabled()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_BUZZER_ON_OUTPUT;
			DataTypeName = "REQ_BUZZER_ON_OUTPUT";
		}
		...
		{
			AidLog logs = new AidLog( "McReqBuzzerEnabled.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					BUZZER_ON_OUTPUT data = BUZZER_ON_OUTPUT.Init();
					data.onflg = (short)( ( true == Enabled ) ? 1 : 0 );
					int retRt64 = 0;
