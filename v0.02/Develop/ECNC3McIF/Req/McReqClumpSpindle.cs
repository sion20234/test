...
{
	/// <summary>4-5-28 スピンドル クランプ／アンクランプ コマンド</summary>
	public class McReqClumpSpindle : McCommBasic, IEcnc3McCommand, IDisposable
    {
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;
		/// <summary>コンストラクタ</summary>
		public McReqClumpSpindle()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_SPCLUMP;
			DataTypeName = "REQ_SPCLUMP";
		}
		...
		{
			AidLog logs = new AidLog( "McReqClumpSpindle.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					SPCLUMP data = SPCLUMP.Init();
