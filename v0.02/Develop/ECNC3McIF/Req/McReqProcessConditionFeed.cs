...
{
#if __KEY_DELETE_20161013__	//	仕様書v1.5よりコマンドが破棄された。
	/// <summary>4-5-4.加工条件による主軸速度通知コマンド</summary>
	public class McReqProcessConditionFeed : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqProcessConditionFeed()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_PRCFEED;
			DataTypeName = "REQ_PRCFEED";
		}
		/// <summary>加工条件による主軸速度</summary>
		public short PrCFeed { get; set; }
		...
		{
			AidLog logs = new AidLog( "McReqProcessConditionFeed.Execute" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					PRCFEED data = PRCFEED.Init();
					data.PrCFeed = PrCFeed;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},PrCFeed={data.PrCFeed}" );
