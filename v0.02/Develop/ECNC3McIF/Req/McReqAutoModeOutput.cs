...
{
	/// <summary>4-5-51.自動モード中出力コマンド</summary>
	public class McReqAutoModeOutput : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqAutoModeOutput()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_AUTOMODE_OUTPUT;
			DataTypeName = "REQ_AUTOMODE_OUTPUT";
		}
		...
		{
			AidLog logs = new AidLog( "McReqAutoModeOutput.ExecuteByValue" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					AUTOMODE_OUTPUT data = AUTOMODE_OUTPUT.Init();
					data.onflg = (short)( ( true == SettingValue ) ? 1 : 0 );
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},onflg={data.onflg}" );
