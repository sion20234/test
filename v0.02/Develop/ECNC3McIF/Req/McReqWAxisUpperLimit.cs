...
{
	/// <summary>4-5-3 W軸上限値設定コマンド</summary>
	public class McReqWAxisUpperLimit : McCommBasic, IEcnc3McCommand, IDisposable
    {
		...
		public McReqWAxisUpperLimit()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_WTOPPOS;
			DataTypeName = "REQ_WTOPPOS";
			_xmlElementName = "Root/Opr/WTopPos";
			_xmlAttributeName = "val";
		}
		/// <summary>W軸上限値</summary>
		...
		{
			return ExecuteByValue( SettingValue, _xmlElementName, _xmlAttributeName );
		}
		/// <summary>コマンド発行</summary>
		...
		{
			AidLog logs = new AidLog( "McReqSettingWAxisUpperLimit.ExecuteByValue" );
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					WTOPPOS data = WTOPPOS.Init();
					data.WTopPos = target;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},WTopPos={data.WTopPos}" );
		...
		{
			if( null != fs ) {
				SettingValue = fs.AttrValue( _xmlElementName, _xmlAttributeName );
			} else {
				using( FileSettings fs0 = new FileSettings() ) {
					SettingValue = fs0.AttrValue( _xmlElementName, _xmlAttributeName );
				}
			}
			return Execute();
		}
