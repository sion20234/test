...
{
	/// <summary>4-5-25.ガイド交換位置移動コマンド</summary>
	public class McReqGuideMove : McCommBasic, IEcnc3McCommand, IDisposable
	{
		/// <summary>Dispose()関数が呼び出し済みであるかどうかのフラグ</summary>
		private bool _disposed = false;

		/// <summary>コンストラクタ</summary>
		public McReqGuideMove()
		{
			ClassName = GetType().Name;
			DataType = Syncdef.REQ_GUIDEEXCHPOS;
			DataTypeName = "REQ_GUIDEEXCHPOS";
		}

		/// <summary>動作</summary>
		public GuidePositions Position { get; set; }
		/// <summary>軸選択</summary>
		public AxisBits Axis { get; set; }
		/// <summary>電極番号</summary>
		public short GuideNumber { get; set; }
		...
		public ResultCodes Execute()
		{
			...
			if( GuidePositions.Unknown == Position ) {
				return ResultCodes.InvalidArgument;
			}
			ResultCodes ret = ResultCodes.McNotInitialize;
			while( true == StandBy() ) {
				try {
					GUIDEEXCHPOS data = GUIDEEXCHPOS.Init();
					data.axis = (short)Axis;
					data.pos = (short)Position;
					data.GuideNo = GuideNumber;
					int retRt64 = 0;
					TechnoMethods method = TechnoMethods.SendCommand;
					logs.Sure( $"SendCommand({DataTypeName},axis={data.axis},pos={data.pos},GuideNo={data.GuideNo})" );
