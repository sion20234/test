...
namespace ECNC3.Models
{
	...
	public class StructureProcessConditionList : List<StructureProcessConditionItem>, IDisposable, ICloneable
	{
			...
			try {
				if( false == _disposed ) {
					if( true == disposing ) {
						//  マネージリソースの解放
						if( 0 < Count ) {
							foreach( StructureProcessConditionItem item in this ) {
								item.Dispose();
							}
							this.Clear();
						}
		...
		public object Clone()
		{
			StructureProcessConditionList temp = new StructureProcessConditionList();
			foreach( StructureProcessConditionItem item in this ) {
				temp.Add( item.Clone() as StructureProcessConditionItem );
			}
			return temp;
		}
#if false
