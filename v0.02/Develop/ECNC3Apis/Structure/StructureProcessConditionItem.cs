...
namespace ECNC3.Models
{
	...
	public class StructureProcessConditionItem : ICloneable, IDisposable
	{
        ...
        ///加工条件内容の有効値チェック
        /// </summary>
        /// <param name="sfipEnable"></param>
        /// <returns></returns>
        public bool IsValid(bool sfipEnable, int n5LowLimit = 100)
        {
            _N5CapBound = n5LowLimit;
            return ((sfipEnable == true)
                ? true 
                    : (
                        (IPVal >= 0) 
                        && (SFIPVal== 0) 
        ...
        {
            bool edited = false;
            if(!((IPVal >= 0) && (SFIPVal == 0)))
            {
                SFIPVal = 0;                //SFIP修正
                IPVal = 3000;               //IP修正
                edited = true;
            }
            if(!(CAPVal > _N5CapBound))
            {
                CAPVal = 0;                 //CAP修正
                edited = true;
            }
		...
		public int Compare( StructureProcessConditionItem source )
		{
			...
			while( true ) {
                ...
                ret = SFIPVal.CompareTo(source.SFIPVal);
                if (0 != ret)
                {
                    break;
                }
