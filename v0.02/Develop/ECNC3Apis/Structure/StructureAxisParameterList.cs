...
namespace ECNC3.Models
{
	...
	public class StructureAxisParameterList : IDisposable
	{
        public StructureAxisParameterList()
        {
            for(int ict = 0; ict < 9; ict++)
            {
                Items[ict] = new StructureAxisParameterItem();
            }
        }
        /// <summary>
        /// サーボパラメーターの軸パラメーター
        /// </summary>
        public StructureAxisParameterItem[] Items = new StructureAxisParameterItem[9];
        public string AxAecSoftLim { get; set; }
			...
			try {
				if( false == _disposed ) {
					if( true == disposing ) {
						//  マネージリソースの解放
						if( 0 < Items.Count()) {
							for( int itemsCt = 0; itemsCt < Items.Count(); itemsCt++ ) {
								Items[itemsCt] = null;
							}
						}
					}
