...
namespace ECNC3.Models
{
	...
	public class FileSettings : FileAccessCommon, IEcnc3Backup, IDisposable
	{
		...
		{
			Name = this.ToString();
			RegistMasterFile( @"CncSys.xml" );
		}
        public FileSettings(string fileName)
        {
            Name = this.ToString();
            RegistMasterFile(@fileName);
        }
		...
		public ResultCodes Write()
		{
			...
			while( true == _edted ) {
				...
				if( null != _xmlDoc ) {
					AidLog logs = new AidLog( "FileSettings.Save" );
					AidXml xml = new AidXml();
        ...
        /// ロードされているXMLノードをファイル出力します。
        /// </remarks>
        public ResultCodes WriteSimple()
        {
            ResultCodes ret = ResultCodes.Success;
            if (null != _xmlDoc)
            {
