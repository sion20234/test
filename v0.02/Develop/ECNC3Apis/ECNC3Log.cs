...
namespace ECNC3.Models.Common
{
	...
	public class ECNC3Log
	{
		...
		public void Operate( string note )
		{
            ...
            OutputOperate($"{FunctionName},OPE,{note}");
        }
        ...
        /// <summary>ログ出力</summary>
		/// <param name="msg">取得される文字列</param>
		private void OutputOperate(string msg)
        {
            try
            {
                //	ログ出力文字列作成
                ...
                {
                    //	ログ出力
                    sw = new StreamWriter(OutputFilePath.Replace(".log", "OPERATION.log"), true, System.Text.Encoding.GetEncoding("UTF-8"));
                    if (null != sw)
                    {
                        sw.WriteLine(output);
                    }
